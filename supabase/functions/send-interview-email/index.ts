// supabase/functions/send-marketing-interview-email/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    const SUPABASE_URL = "https://hbtjbmgreguchdozyxwb.supabase.co";
    const SERVICE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhidGpibWdyZWd1Y2hkb3p5eHdiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MTk1NTQ2OCwiZXhwIjoyMDg3NTMxNDY4fQ.Vojzv80ibdsWO5iewRRsrVlIe55jP_F9r6GW2rIKels";
    const RESEND_API_KEY = "re_J5XbQs93_MHNvfvY3ki6MxhnTJ7hUYbmv";
    const FROM_EMAIL = "noreply@interview.azzurrohotels.com";
    ...
    const clipsHtml = signedLinks.map((l) => {
      ...
      return `
        <div style="padding:12px 14px;border:1px solid #e6eef5;border-radius:12px;margin:10px 0;background:#ffffff;">
          ...
          <div style="margin-top:10px;">
            <a href="${l.url}" style="display:inline-block;padding:10px 12px;border-radius:10px;background:linear-gradient(135deg,#8b5cf6,#06d6c8);color:#fff;text-decoration:none;font-weight:900;">Play recording</a>
          </div>
        </div>
      `;
    }).join("");

    const subject = `New Marketing Interview Submission — ${interview.candidate_name}`;
    ...
    const html = `
      <div style="font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial;background:#f3f9ff;padding:24px;">
        <div style="max-width:760px;margin:0 auto;background:#ffffff;border:1px solid #e6eef5;border-radius:16px;overflow:hidden;">
          <div style="padding:18px 20px;background:linear-gradient(135deg,rgba(139,92,246,0.12),rgba(6,214,200,0.10));border-bottom:1px solid #e6eef5;">
            <div style="font-weight:900;font-size:16px;color:#0b1b2b;">CBIT • AI Interview Portal – Marketing</div>
          ...
          <div style="padding:18px 20px;">
            ...
            ${speedHtml}
            <div style="margin-top:16px;font-weight:900;color:#0b1b2b;">Recordings</div>
            <div style="color:#4b5b6a;margin-top:4px;">Each question is a separate clip.</div>
            ${clipsHtml}
            <div style="margin-top:14px;color:#4b5b6a;font-size:12px;">Links expire in 7 days.</div>
          </div>
        </div>
      </div>
    `;
